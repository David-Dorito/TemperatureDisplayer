...
static Pcd8544_Status SPI_TransmitData_Bridge(void* pSpiHandle, uint8_t* pTxBuffer, uint16_t len,
                                              uint32_t timeout);
static void           GPIO_WritePin_Bridge(void* pHandle, u8 isEnabled);
static void I2C_MasterTransmitData_Bridge(void* pI2cHandle, u16 slaveAddr, u8 addrMode, uint8_t* pTxBuffer,
                                          u16 len);
static void I2C_MasterReceiveData_Bridge(void* pI2cHandle, u16 slaveAddr, u8 addrMode, uint8_t* pRxBuffer,
                                         u16 len);
...
static Pcd8544_PlatformDrivers lcdTransport = {
    .SpiTransmit = SPI_TransmitData_Bridge,
    .GpioWritePin = GPIO_WritePin_Bridge,
    .Delay = UnpreciseDelay,
};

Pcd8544_Handle lcdHandle = (Pcd8544_Handle){
    .FrameBuffer = pLcdFrameBuffer,
    .Spi = &lcdSpiHandle,
    .CsPin = &lcdSelectPin,
    .DcPin = &lcdDcPin,
    .ResPin = &lcdResetPin,
    .LedPin = &lcdBacklightPin,
    .VccPin = NULL,
